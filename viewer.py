...
class HabitatSimInteractiveViewer(Application):
    ...
    def reconfigure_sim(self) -> None:
        ...
        ########### update to run on non CUDA system ######

        # configure our sim_settings but then set the agent to our default
        self.cfg = make_cfg(self.sim_settings)

    	# Check and directly disable physics if possible

        if hasattr(self.cfg.sim_cfg, 'enable_physics'):
            logger.info("Disabling physics in simulator...")
            self.cfg.sim_cfg.enable_physics = False  # Disable physics directly in sim_cf
        else:
            logger.warning("No 'enable_physics' attribute found in sim_cfg.")
